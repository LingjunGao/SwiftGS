...
#
# If you run it with "bash ./setup_swiftgs.sh", installation still works,
...
echo "[5/6] Installing SwiftGS from source..."
...
echo "[7/7] Downloading Benchmark_Training data from Zenodo..."
RESULTS_DIR="${SCRIPT_DIR}/examples/results"
BENCHMARK_URL="https://zenodo.org/records/18943926/files/Benchmark_Training.tar.gz?download=1"
BENCHMARK_ARCHIVE="${RESULTS_DIR}/Benchmark_Training.tar.gz"

mkdir -p "${RESULTS_DIR}"
echo "Downloading Benchmark_Training.tar.gz to ${RESULTS_DIR} ..."
curl -L --progress-bar --retry 5 \
    -o "${BENCHMARK_ARCHIVE}" \
    "${BENCHMARK_URL}"
...
echo "Benchmark data extracted to ${RESULTS_DIR}"
...
if [[ ${is_sourced} -eq 1 ]]; then
  ...
  echo "Tip: use 'source ./setup_swiftgs.sh' next time."
fi
